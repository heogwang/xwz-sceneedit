...
class arcvec // simple 3D vector class
{
  public:
    vec_float x,y,z;

    arcvec() {}
    arcvec( vec_float xx, vec_float yy, vec_float zz )
    { x=xx; y=yy; z=zz; }

    inline arcvec operator + (arcvec t) // addition
    { return arcvec(x+t.x,y+t.y,z+t.z); }
    inline arcvec operator - (arcvec t) // subtraction
    { return arcvec(x-t.x,y-t.y,z-t.z); }
    inline arcvec operator * (vec_float t) // dot product
    { return arcvec(x*t,y*t,z*t); }
    inline vec_float operator * (arcvec t) // scalar product
    { return x*t.x + y*t.y + z*t.z; }
    inline arcvec operator ^ (arcvec t) // cross product
    { return arcvec( y*t.z-z*t.y, t.x*z-x*t.z, x*t.y-y*t.x ); }

    inline vec_float length() // pythagorean length
    { return sqrt(x*x + y*y + z*z); }
    inline arcvec unit() // normalized to a length of 1
    { vec_float l = length();
      if (l == 0.0) return arcvec(0.0,0.0,0.0);
      return arcvec(x/l,y/l,z/l); }
    inline bool zero() // returns true if a zero vector
    { return x==0 && y==0 && z==0; }
    inline bool equals(arcvec t) // returns true if exactly equal
    { return x==t.x && y==t.y && z==t.z; }
};

extern void arcball_setzoom(float radius, arcvec eye, arcvec up);
